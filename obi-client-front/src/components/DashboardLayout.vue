...
import DashboardContextModal from './DashboardContextModal.vue';
...
const loadDashboards = async () => {
  try {
    ...
    dashboards.value = Array.isArray(res) ? res : (res?.data || []);
...
const selectDashboard = async (dash) => {
    ...
    try {
        ...
        dashboardReports.value = Array.isArray(res) ? res : (res?.data || []);
    } catch (e) {
        console.error(e);
    } finally {
...
<template>
  <div class="flex h-[calc(100vh-80px)] bg-slate-100">
    ...
    <InstrumentEditor
        ...
        :dashboard-context="activeDashboard.context_definition"
        :initial-report="reportToEdit"
        @close="onEditorClose"
        @saved="onEditorSaved"
    />

    <DashboardContextModal
